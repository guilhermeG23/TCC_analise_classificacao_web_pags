...
def ler_quantidade_variavel(entrada):
    return len(entrada)
...
#Retorno de data para formatacao de pasta
...
def ler_pagina(entrada):
    saida = ""
    with open("paginas/{}.txt".format(entrada)) as arquivo:
        for x in arquivo:
            saida = saida + x
        arquivo.close()
    return saida

def ler_todas_paginas(paginas):
    todas_paginas = []
    for pagina in paginas:
        todas_paginas.append(ler_pagina(pagina))
...
def limpar_quebra_linha(entrada):
    entrada = entrada.replace("\n", "")
    return entrada
...
def todas_versoes_software():
    with open("versoes/versoes.txt", encoding="utf-8") as arquivo:
        saida = []
        for x in arquivo:
            saida.append(split_geral(x, "|"))
        arquivo.close()
    return saida
